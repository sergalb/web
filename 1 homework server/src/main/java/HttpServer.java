...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException, NoSuchAlgorithmException {
        ...
        if (file.isFile()) {
            ...
            if (request.headers.containsKey("If-None-Match") && request.headers.get("If-None-Match").equals(eTag)) {
    ...
    private String getEtag(byte[] body) throws NoSuchAlgorithmException {
        MessageDigest messageDigest = MessageDigest.getInstance("SHA-256");
        byte[] digest = messageDigest.digest(body);
    ...
    private static final class Request {
        ...
        private Request(String request) throws UnsupportedEncodingException {
            ...
            String startUri = firstLineTokens[1];
            if (!startUri.endsWith(".htm") && !startUri.endsWith(".html") && !startUri.endsWith(".png")) {
                startUri = startUri + "/index.html";
            }
            uri = URLDecoder.decode(startUri, "UTF-8");
            headers = new HashMap<>();
